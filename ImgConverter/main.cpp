...
int main(int argc, const char** argv) {
    ...
    unique_ptr<ImageFormatInterface> in_format(GetFormatInterface(in_path));
    unique_ptr<ImageFormatInterface> out_format(GetFormatInterface(out_path));

    if (in_format.get() == nullptr) {
        cerr << "Unknown format of the input file"sv << endl;
        return 2;
    }
    if (out_format.get() == nullptr) {
        cerr << "Unknown format of the output file"sv << endl;
        return 3;
    }

    img_lib::Image image = in_format.get()->LoadImage(in_path);
    if (!image) {
        cerr << "Loading failed"sv << endl;
        return 4;
    }

    if (!out_format.get()->SaveImage(out_path, image)) {
